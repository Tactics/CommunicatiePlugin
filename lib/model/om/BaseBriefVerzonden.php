...
abstract class BaseBriefVerzonden extends BaseObject  implements Persistent {
	...
	public function getBatchtaakId()
	{

		return $this->batchtaak_id;
	}
	...
	public function setBatchtaakId($v)
	{
		...
		if ($this->batchtaak_id !== $v) {
			$this->batchtaak_id = $v;
			$this->modifiedColumns[] = BriefVerzondenPeer::BATCHTAAK_ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->custom = $rs->getBoolean($startcol + 12);

			$this->culture = $rs->getString($startcol + 13);

			$this->status = $rs->getString($startcol + 14);

			$this->batchtaak_id = $rs->getInt($startcol + 15);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[10] => $this->getCc(),
			$keys[11] => $this->getBcc(),
			$keys[12] => $this->getCustom(),
			$keys[13] => $this->getCulture(),
			$keys[14] => $this->getStatus(),
			$keys[15] => $this->getBatchtaakId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[12], $arr)) $this->setCustom($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setCulture($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setStatus($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setBatchtaakId($arr[$keys[15]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BriefVerzondenPeer::BATCHTAAK_ID)) $criteria->add(BriefVerzondenPeer::BATCHTAAK_ID, $this->batchtaak_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setBatchtaakId($this->batchtaak_id);
